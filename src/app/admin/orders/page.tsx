    ...
    <div className="max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
        ...
        <button
          ...
          onClick={fetchOrders}
          disabled={loading}
          className="btn-primary flex items-center justify-center gap-2 !px-6 !py-3 text-sm disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {[
          ...
          },
          {
            ...
            value: orders.filter((o) => o.status === 'contacted').length,
      ...
      {loading ? (
        ...
        <div className="premium-card text-center py-16">
          <ClipboardList size={56} className="mx-auto text-foreground/20 mb-4" />
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-brand-primary/10 text-foreground/40 text-xs uppercase tracking-widest">
                  ...
                  <th className="px-6 py-4 font-bold">{t('table.location')}</th>
                  <th className="px-6 py-4 font-bold">{t('table.crane')}</th>
              ...
              <tbody>
                {orders.map((order) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-start gap-2 max-w-xs text-foreground/80">
                        <MapPin
                          size={16}
                          className="text-foreground/30 shrink-0 mt-0.5"
                        />
                        {order.location}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-foreground/80">
                        <Truck size={16} className="text-foreground/30" />
                        {order.craneModel || '—'}
                      </div>
                    </td>
